...
export default function Products() {
  ...
  const list = useMemo(
    () =>
      data.products
        .filter((p) => (showInactive || p.active) && (!group || p.group_name === group) && matches(q, p.name))
        .sort((a, b) => GROUPS.indexOf(a.group_name ?? "") - GROUPS.indexOf(b.group_name ?? "") || a.name.localeCompare(b.name, "pl")),
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 space-y-3">
      <div className="flex items-center gap-2">
        <h1 className="text-lg font-semibold mr-auto">Produkty</h1>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex-1 min-w-40">
          <SearchInput value={q} onChange={setQ} placeholder="Nazwa" />
        </div>
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
        {list.map((p) => {
          ...
          return (
            <button key={p.id} type="button" className={`card overflow-hidden text-left hover:border-gray-500 ${p.active ? "" : "opacity-50"}`} onClick={() => setEditing(p)}>
